...
from models_search.shared_gan_2stage import GeneratorNoAttnskip,Discriminator
from models_search.controller import Controller,Controller3,ControllerNoAttn,ControllerSkipMore
...
def create_shared_gan(args, weights_init, cur_stage=1):
    gen_net = GeneratorNoAttnskip(ngf=args.gf_dim, nz=args.latent_dim, nc=3, im_size=args.im_size, cur_stage=cur_stage).cuda()
...
def main():
    ...
    loader = DataLoader(dataset_a, batch_size=16, num_workers=8)
    ...
    for i, imgs in enumerate(loader):
        for j, img in enumerate(imgs):
            vutils.save_image(img,
                              os.path.join(dist,
                                           '%d.png' % (i * 16 + j)))  # , normalize=True, range=(-1,1))
    ...
    for search_iter in tqdm(range(int(start_search_iter), int(args.max_search_iter)), desc='search progress'):
        ...
        dynamic_reset = train_shared_mixed_NoCL(args, gen_net, dis_net, g_loss_history,
                                           ...
                                           dis_optimizer, dataloader, cur_stage=cur_stage)
        train_controller(args, controller_skip, ctrl_skip_optimizer, gen_net, im_nums, dist, writer_dict, cur_stage=cur_stage)
    ...
    final_archs_fid = get_topk_arch_hidden(args, controller_skip, gen_net, dist)
