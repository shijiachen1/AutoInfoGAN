...
args = cfg.parse_args()
n_sample,im_size = args.n_sample,args.im_size
...
def _handle_path(path, sess, im_nums, low_profile=False):
    if path.endswith('.npz'):
        ...
        if low_profile:
            m, s = calculate_activation_statistics_from_files(files[:im_nums], sess)
        else:
            # x = np.array([imread(str(fn)).astype(np.float32) for fn in files])
            x = np.array([random_crop(imread(str(fn)).astype(np.float32), im_size) for fn in files[:im_nums]])
            m, s = calculate_activation_statistics(x, sess)
            del x  # clean up memory
    return m, s

def _handle_path_numbers(path, sess, n_samples, low_profile=False):
    if path.endswith('.npz'):
        f = np.load(path)
        m, s = f['mu'][:], f['sigma'][:]
        f.close()
        ...
        files = list(path.glob('*.jpg')) + list(path.glob('*.png'))
        if low_profile:
            m, s = calculate_activation_statistics_from_files(files[:n_samples], sess)
        else:
            x = np.array([imread(str(fn)).astype(np.float32) for fn in files[:n_samples]])
            m, s = calculate_activation_statistics(x, sess)
            del x  # clean up memory
    return m, s

def calculate_fid_given_paths(paths, im_nums, inception_path, low_profile=False):
    ...
    with tf.Session(config=config) as sess:
        ...
        m1, s1 = _handle_path(paths[0], sess, im_nums, low_profile=low_profile)
        m2, s2 = _handle_path(paths[1], sess, im_nums, low_profile=low_profile)
